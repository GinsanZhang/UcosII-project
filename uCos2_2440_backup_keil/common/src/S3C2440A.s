...
IRQ_Entry
	...
	IMPORT Nand_Init
    IMPORT Nand_Copy2SDRAM
	EXPORT  HandleEINT0	
	IMPORT |Image$$ER_ROM1$$Limit|
    IMPORT |Image$$ER_ROM2$$Length|
	IMPORT __rt_entry_sh
...
;  Enter User Mode and set its Stack Pointer		   ；不用用户模式
               ; MSR     CPSR_c, #Mode_USR
              ;  MOV     SP, R0
             ;   SUB     SL, SP, #USR_Stack_Size

;在SDRAM运行时需要拷贝异常表和相应的跳转表到0地址,run in SDRAM, used by testing
;				LDR		R0, =Vectors
;				ADD 	R3, R0, #128
;				MOV 	R2, 0x0
;0	
...
;Check boot mode
	...
	beq	nandflash		; read nand flash


;从norfalsh启动，拷贝代码到SDRAM中运行
;norflash				;利用分散加载文件，加载代码可以自动生成，会自动去拷贝加载地址不等于运行地址的代码
;		LDR		R0, =|Image$$ER_ROM1$$Limit|
;				ADD 	R3, R0, #32*1024
;				MOV 	R2, 0x30000000
;1	
;				LDMIA	R0!, {R4-R7}
...
;从Nandfalsh启动，拷贝代码到SDRAM中运行

nandflash 
            BL Nand_Init
		    ldr r0, =0x30000000
			ldr r1, =|Image$$ER_ROM1$$Limit|                        
			ldr r2, =|Image$$ER_ROM2$$Length|   
          	BL	Nand_Copy2SDRAM            			         
			tst r0, #0x0                   
			bne nandflash
			ldr R0, =__rt_entry_sh
			BX	R0
